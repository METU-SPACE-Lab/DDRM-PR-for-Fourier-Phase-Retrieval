...
def load_checkpoint(
    model, optimizer, checkpoint_dir, epoch_loss=False, CV_psnrs=False, CV_ssims=False
):
...
def fienup_phase_retrieval(
    ...
    assert (
        mode == "input-output" or mode == "output-output" or mode == "hybrid"
    ...
    for i in range(1, steps + 1):
        ...
        # or are not masked
        indices = cp.logical_or(cp.logical_and(y < 0, mask), cp.logical_not(mask))
...
def random_best(magnitudes_oversampled___mask___mask_slice___normalization):
    ...
    magnitudes_oversampled, mask, mask_slice, normalization = (
        magnitudes_oversampled___mask___mask_slice___normalization
    )
    ...
    for i in range(50):
        ...
        result_oversampled = cp.array(
            fienup_phase_retrieval(
                magnitudes_oversampled,
                steps=50,
                mask=mask,
                verbose=False,
        ...
        resid = LA.norm(
            magnitudes_oversampled
            - cp.asnumpy(cp.fft.fft2(result_oversampled) / (normalization)),
            2,
        )
        if resid < resid_best:
            resid_best = resid
...
def psnr_correct_channelwise(image_tocorrect, image_groundtruth):
    for i in range(3):
        if skimage.metrics.peak_signal_noise_ratio(
            np.asarray(image_tocorrect[i], dtype=np.uint8),
            np.flip(np.asarray(image_groundtruth[i], dtype=np.uint8)),
        ) > skimage.metrics.peak_signal_noise_ratio(
            np.asarray(image_tocorrect[i], dtype=np.uint8),
            np.asarray(image_groundtruth[i], dtype=np.uint8),
        ):
...
def hio_stage(magnitudes_oversampled___mask___mask_slice___normalization___image):
    magnitudes_oversampled, mask, mask_slice, normalization, image_ = (
        magnitudes_oversampled___mask___mask_slice___normalization___image
    )

    for i in range(3):
        magnitudes_oversampled___mask___mask_slice___normalization_perchannel = (
            magnitudes_oversampled[i],
            mask,
            mask_slice,
            normalization,
        )
        x_init_best = random_best(
            magnitudes_oversampled___mask___mask_slice___normalization_perchannel
        )

        result_oversampled = fienup_phase_retrieval(
            magnitudes_oversampled[i],
            steps=1000,
            mask=mask,
            verbose=False,
    ...
    return (
        torch.tensor(image_iter, device=torch.device("cuda"), dtype=torch.float32)
        / 255
        * 2
        - 1
    )
...
def pr_encode(image_full, alpha_=3):
    ...
    for i in range(3):
        image_ = image_full[i]
        SamplingRateSqrt = 2
        padded_zero = (np.array(image_.shape) * (SamplingRateSqrt - 1) / 2).astype(
            np.int
        )
        ...
        mask_slice = (
            slice(padded_zero[0], padded_zero[0] + image_.shape[0]),
            slice(padded_zero[1], padded_zero[1] + image_.shape[1]),
        )
        ...
        # add noise
        alpha = alpha_
        intensity_noise = alpha * np.multiply(
            np.abs(fourier_oversampled_), np.random.randn(*fourier_oversampled_.shape)
        )
...
def jd(magnitudes_oversampled___mask___mask_slice___normalization___image_):
    magnitudes_oversampled, mask, mask_slice, normalization, image_, image_full = (
        magnitudes_oversampled___mask___mask_slice___normalization___image_
    )
    ...
    image_iter[0, :, :, :] = psnr_correct_channelwise(
        image_iter[0, :, :, :], image_full
    )
