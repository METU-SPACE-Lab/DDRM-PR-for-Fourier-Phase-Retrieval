...
from functions.utils import load_data, load_checkpoint, cal_ssim, cmap_convert, hio_stage
    ...
    def sample_sequence(self, model, cls_fn=None):
        ...
        for x_orig, classes in pbar:
            ...
            for eta in np.arange(0.7, 1.0, 0.08):
                for etaB in np.arange(0.17, eta * 2 / 3, 0.1):
            ...
            for self.args.eta, self.args.etaB, self.args.num_avg_samples, self.args.timesteps in random_parameters:            
                avg_psnr = 0.0
                avg_psnr_y = 0.0
                ...
                for i in [-1]: # range(len(x)):
                    for j in range(x[i].size(0)):
                        ...
                        if i == len(x)-1 or i == -1:
                            ...
                            avg_psnr += psnr
                            if deg[:4] == 'jpeg' or deg == 'pr':
                                y_inv = inverse_data_transform(config, pinv_y_0[j])
                                mse = torch.mean((y_inv - orig) ** 2)
                                psnr = 10 * torch.log10(1 / mse)
                                avg_psnr_y += psnr
                
                txt_write = f"{idx_so_far + j} {self.args.eta}, {self.args.etaB}, {self.args.num_avg_samples}, {self.args.timesteps}, {self.args.init_timestep}, {idx_so_far + j} :: {avg_psnr.data}, {avg_psnr_y.data}\n"
                all_combinations.append(txt_write)
                with open('allcomb.txt', 'a') as f:
                    f.write(txt_write)
                print(self.args.eta, self.args.etaB, self.args.num_avg_samples, self.args.timesteps, self.args.init_timestep, ":", avg_psnr.data, avg_psnr_y.data," \n \n",)
