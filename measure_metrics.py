...
def calculate_metrics(folder_path):
    ...
    ssim_metric = StructuralSimilarityIndexMeasure(gaussian_kernel=True, sigma=1.5, kernel_size=11, reduction='elementwise_mean')
    ...
    for i in range(11):  # Assuming the range of X is from 0 to 10
        ...
        orig_img_tensor = torch.tensor(orig_img).permute(2, 0, 1).unsqueeze(0).float() / 255.0
        method_a_img_tensor = torch.tensor(method_a_img).permute(2, 0, 1).unsqueeze(0).float() / 255.0
        method_b_img_tensor = torch.tensor(method_b_img).permute(2, 0, 1).unsqueeze(0).float() / 255.0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate average PSNR, SSIM, and LPIPS for two methods.")
